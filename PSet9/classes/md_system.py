...
class MDSystem:
    ...
    def __init__(self):
        self.num_particle = 4     # number of particles in the system
        ...
        self.init_vel = 10 # initial total velocity of the particles
    ...
    def update_rel_pos(self):
        ...
        for i in range(2):  # find particle correlation within r_c for x and y
            for x_ind, x in enumerate(self.dots[:, i]):
                ...
                reflection_out = np.any([np.absolute(rel_pos - self.size) > r_c,
                    np.absolute(rel_pos + self.size) > r_c], 0) # reflection out of r_c
                reflection_in = np.any([np.absolute(rel_pos - self.size) < r_c,
                    np.absolute(rel_pos + self.size) < r_c], 0)  # reflection inside r_c
    ...
    def calc_accel(self):
        ...
        rel_dist_sq = self.dist_data[:, :, 0] ** 2 + \
            self.dist_data[:, :, 1] ** 2
        ...
        for i in range(2):
            for j in range(self.num_particle):
                accel[j, i] = np.sum(-4 * (-12 / rel_dist_sq[j, non_zero[j]] ** 7 +
                                    6 / rel_dist_sq[j, non_zero[j]] ** 4) * \
    ...
    def energy(self):
        ...
        return self.kinetic() + \
            np.sum(potential(dist_mat[dist_mat != 0])) / 2
